...
from src.vectorizer.auto_vector import Auto_Simple_Vec as Auto_Simple_Vec
from src.vectorizer.two_layer_autoencoder_vector import AutoCoder as AutoCoder
...
class Summarizor():
    ...
    def __init__(self,MyVec):
        ...
        self.name = "Fourth Version " + self.vectorizer.name
        self.dist = Dist()
        self.disttype =Distance.EUD
        self.itimes= 20
        self.count_index = 0
        self.info =  self.name + ", iter_times=" + str(
            self.itimes) + ", vector=" + self.vectorizer.name + ", distance=" + str(self.disttype)
        self.cluewords =set()
        self.load_clue_words()
        self.target_tag = ["n","v","m"]
        self.clue_weight = 0.3
        self.useless = [[],[]]

    def set_parameter(self,iter_times,disttype):
        self.itimes = iter_times
        self.vectorizer.set_parameter(self.itimes)
    ...
    def other_values(self,sens_words):
        ovalue = {}
        for i in range(len(sens_words)-1):
            ovalue[i] = 0
            for j in range(i,len(sens_words)):
                if len(set(sens_words[i]).intersection(set(sens_words[j])))>0:
                    ovalue[i]+=1
            ovalue[i]/=(i+1)
        ovalue[len(sens_words)-1] = 0
        return ovalue
    ...
    def score_option(self,option,coverage_list,relative_matrix,clues_list,entities_list):
        ...
        score = coverage_value + relative_value + clues_value + entities_value
    ...
    def save_value(self,path,key,coverage_list,relative_matrix,clues_list,entities_list):
        ftools.check_filename(path)
        save_dict = {}
        save_dict[key] =[coverage_list,relative_matrix,clues_list,entities_list]
    ...
    def summarize(self,text,num =3,fname=None):
        ...
        clues_list = self.other_values(sens_words)
        entities_list = self.entities_values(sens_words,sens_tag)
        options = self.generate_options(len(sens),num)
        # print(''.join(text))
        # tkey = tools.md5(''.join(text))
        # print(tkey)
        # if tkey in self.useless[0]:
        #     print(text)
        #     print(self.useless[1][self.useless[0].index(tkey)])
        #     input()
        # self.useless[0].append(tkey)
        # self.useless[1].append(text)
        self.save_value(Dir.res+"/encoder/"+self.name+"/"+fname,fname,coverage_list,relative_matrix,clues_list,entities_list)
